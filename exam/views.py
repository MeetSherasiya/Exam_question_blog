...
Fields1 = Paper.objects.values_list('Field', flat=True).distinct().order_by('Field')
branch1 = Paper.objects.values_list('Field', 'branch').distinct().order_by('branch')
year1 = Paper.objects.values_list('Field', 'branch', 'year').distinct().order_by('year')
exam_type1 = Paper.objects.values_list('Field', 'branch', 'year', 'exam_type').distinct().order_by('exam_type')
semester1 = Paper.objects.values_list('Field', 'branch', 'year', 'exam_type','semester').distinct().order_by('semester')
subject1 = Paper.objects.values_list('Field', 'branch', 'year', 'exam_type','semester', 'subject').distinct().order_by('subject')

# all = Paper.objects.values_list('Field','branch','year','exam_type','semester','subject').distinct()
# print(all)

drp = {'Fields': Fields1, 'branch': branch1, 'year': year1, 'exam_type': exam_type1, 'semester': semester1, 'subject': subject1 }

# Create your views here.
def index(request):
    table = Paper.objects.values('Field').distinct()
    context = {'objs':table, 'dropdown': drp}
    return render(request, 'index.html',context)

def field(request,pk):
    table = Paper.objects.filter(Field=pk).values('Field','branch').distinct()
    context = {'objs': table, 'dropdown': drp}
...
def paper(request,pk):
    ...
    context = {'table':table,'sidebar':table1, 'dropdown': drp}
...
def question(request, pk, pk1, pk2, pk3, pk4, pk5):
    ...
    context = {'objs':table,'nav':all_set, 'dropdown': drp}
...
def branch(request, pk, pk1):
    ...
    context = {'objs':all_year,'nav':all_set, 'dropdown': drp}
...
def sem(request, pk, pk1, pk2):
    ...
    context = {'objs':all_value,'nav':all_set, 'dropdown': drp}
